...
This repository provides a minimal reproduction script demonstrating how the OpenAI ChatGPT Codex backend (`https://chatgpt.com/backend-api/codex/responses`) gates **Fast Mode (`service_tier: "priority"`)** throughput behind specific client identification headers.
...
The backend accepts `service_tier: "priority"` without returning an error in both cases, but silently routes non-Codex client headers to standard-speed instances.
...
The benchmark issues requests to `https://chatgpt.com/backend-api/codex/responses` asking for integers `1` to `300` (~603 output tokens), alternating between 4 test conditions:
